        ...
        if pooling == "panpool":
            for _ in range(num_layers_pre):
                self.conv_layers_pre.append(
                    PANConv(in_channels, hidden_channels, filter_size=2)
                )
                in_channels = hidden_channels
        else:
            for _ in range(num_layers_pre):
                mlp = MLP(
                    [in_channels, hidden_channels, hidden_channels], act=activation
                )
    ...
    def forward(self, data):
        ...
        if self.pooling in ["diffpool", "mincut", "dmon", "dense-random"]:
            ...
            if self.pooling == "graclus":
                ...
                complement = batched_negative_edges(
                    edge_index=adj, batch=batch, force_undirected=True
                )
