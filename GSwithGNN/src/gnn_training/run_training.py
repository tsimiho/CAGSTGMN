...
def main():
    ...
    s = (
        ...
        + str(
            cfg_u.makePath(
                cfg["WorkParameter"]["summary_file"]
                + "_"
                + gs_model
                + "_"
                + model_name
    ...
    s1 = (
        ...
        + str(
            cfg_u.makePath(
                cfg["WorkParameter"]["checkpoint_file"]
                + "_"
                + gs_model
                + "_"
                + model_name
    ...
    print("Load data!")
    load_data = utils.load_data_fold(
        load_dir, train_fold + val_fold + test_fold, load_dsc_file
    )
    ...
    if model_name == "mlp":
        ...
        model = rgcn.RGCN(
            guard, num_classes, num_features, hidden_channels=hidden_layer
        )
        optimizer = torch.optim.Adam(
            model.parameters(), lr=learning_rate, weight_decay=weight_decay
        )
