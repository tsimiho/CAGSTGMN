...
def main():
    ...
    gs = gsg.graph_summary_generator(
        gs=cfg.getint("GraphSummary", "GS-Model"),
        max_items=max_items,
    ...
    if bloomfilter:
        file_name += (
            ...
            + str(max_items)
            + "_"
            + cfg.get("GraphSummary", "error_rate")
        )
    ...
    if load_data:
        ...
        if run_dyldo_filter:
            print(f"raw datafile: {raw_datafile}")
            ff.filter(
                raw_datafile,
                filtered_datafile,
                trashed_datafile,
                preSkolemize=preSkolemize,
                numLines=numLines,
            )

        if calc_gs:
            gs.create_graph_information(
                filtered_datafile, k_folds, str(base_dir / "subject_list")
            )
