...
from utils.plot_class_distribution import *
...
class graph_summary_generator:
            ...
            while True:
                ...
                try:
                    ...
                    if s in self.graph_information_:
                        ...
                        self.graph_information_[s] = subject_information(
                            p, o, self.subject_index.get(s)
                        )
    ...
    def calculate_graph_summary(self, bloomfilter):
        ...
        for element in self.graph_information_.items():
            ...
            element[1].weight = int(
                (1 - (len(self.label_dict_[element[1].hash]) / self.num_vertices_))
                * 100
            )
        self.weights_ = [
            element[1].weight for element in self.graph_information_.items()
        ]
            ...
            weights.append(
                int(
                    (
                        1
                        - (
                            len(self.label_dict_[self.graph_information_[element].hash])
                            / num_vertices
                        )
                    )
                    * 100
    ...
    def based_collection_impl(self, prop, typ, bloomfilter):
        ...
        for gi in self.graph_information_.items():
            ...
            if bloomfilter:
                bloom = BloomFilter(
                    max_elements=self.max_items, error_rate=self.error_rate
                )
    ...
    def schemex_impl(self, bloomfilter):
        ...
        for gi in self.graph_information_.items():
            ...
            for h in gi[1].edges:
                ...
                if (
                    self.is_rdf_type(pfeature) == False
                    and h[1] in self.graph_information_.keys()
                ):
    ...
    def element_process_impl(self, element, id_list, num_features, data):
        ...
        for i in element[1].edges:
            ...
            if i[1] not in cache_id_list:
                ...
                data_cache.x = torch.cat(
                    (data_cache.x, torch.zeros(1, num_features)), 0
                )
                # data_cache.y = torch.cat( ( data_cache.y, torch.zeros( 1, dtype=torch.long ) ), 0 )
            ...
            data.edge_index = torch.cat(
                (data.edge_index, torch.tensor([[s_index], [o_index]])), 1
            )
            data.edge_attr = torch.cat((data.edge_attr, torch.tensor([p_index])), 0)

            data_cache.edge_index = torch.cat(
                (
                    data_cache.edge_index,
                    torch.tensor([[cache_s_index], [cache_o_index]]),
                ),
                1,
            )
            data_cache.edge_attr = torch.cat(
                (data_cache.edge_attr, torch.tensor([p_index])), 0
            )
    ...
    def save_fold(
        self, fold, path, save_fold_percentage=1, desc_file="description.csv"
    ):
        """
        ...
        for index, e in enumerate(k_folds_trunc):
            ...
            if self.extract_subgraph:
                ...
                print("root time: {:.02f};".format(end - start))
            ...
            print(
                "for-loop time: {:.02f}; Time overall {:.02f}; ETA s {:.02f}; ETA h {:.02f}; ETA d {:.02f}".format(
                    ep_diff,
                    overall_timediff,
                    (overall_timediff / (index + 1)) * remaining,
                    (overall_timediff / (index + 1)) * remaining / 3600,
                    (overall_timediff / (index + 1)) * remaining / 3600 / 24,
                )
            )
    ...
    def save_fold_schemex(
        self,
        fold,
        path,
        sub_data,
        save_fold_percentage=1,
        edge_sampling_percentage=1.0,
        min_support_classes=0,
        mini_batch=500,
        ...
        k_folds_trunc = [
            e
            for e in k_folds_trunc
            if len(self.label_dict_[self.graph_information_[e].hash])
            > min_support_classes
        ]
        print(
            "Reduce",
        ...
        print(
            ...
            len(k_folds_trunc),
            "to",
            len(degree_filtered_e),
            "because of batch size ",
            mini_batch,
        )
        ...
        for index, e in enumerate(k_folds_trunc):
            ...
            old_edges = len(edges)
            edges = random.sample(
                edges, math.ceil(len(edges) * edge_sampling_percentage)
            )
                ...
                for index_j, j in enumerate(edges):
                    if self.graph_information_.get(j[1]) != None:
                        ...
                        data.edge_attr = torch.cat(
                            (data.edge_attr, data_o.edge_attr), 0
                        )
                        data_o.edge_index += index_offset
                        data_o.edge_index[0, :] = index_j + 1
                        data.edge_index = torch.cat(
                            (data.edge_index, data_o.edge_index), 1
                        )
            ...
            remaining = len(k_folds_trunc) - index + 1
            print(
                "root_time: {:.02f}; hop-time: {:.02f};for-loop time: {:.02f}; Time overall {:.02f}; ETA s {:.02f}; ETA h {:.02f}; ETA d {:.02f}".format(
                    (end - start),
                    (end_n - start_n),
                    ep_diff,
                    overall_timediff,
    ...
    def plot_class_distribution(
        self, filename="class_distribution.png", max_classes=1000
    ):
        """
        Plot class distribution
